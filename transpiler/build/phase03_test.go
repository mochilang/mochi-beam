...
// TestPhase3Collections runs the Phase 3 fixture suite.
// It walks tests/transpiler3/beam/fixtures/phase03/ and asserts that
// each fixture's escript stdout matches expect.txt byte-for-byte.
func TestPhase3Collections(t *testing.T) {
	root := repoRoot(t)
	fixturesDir := filepath.Join(root, "tests", "transpiler3", "beam", "fixtures", "phase03")
	...
	if ran == 0 {
		t.Fatal("no fixtures found in phase03/")
	}
}
