...
// TestPhase4Records runs the Phase 4 fixture suite.
// It walks tests/transpiler3/beam/fixtures/phase04/ and asserts that
// each fixture's escript stdout matches expect.txt byte-for-byte.
func TestPhase4Records(t *testing.T) {
	root := repoRoot(t)
	fixturesDir := filepath.Join(root, "tests", "transpiler3", "beam", "fixtures", "phase04")
	...
	if ran == 0 {
		t.Fatal("no fixtures found in phase04/")
	}
}
