...
// TestPhase7QueryDSL runs the Phase 7 fixture suite.
// It walks tests/transpiler3/beam/fixtures/phase07/ and asserts that
// each fixture's escript stdout matches expect.txt byte-for-byte.
func TestPhase7QueryDSL(t *testing.T) {
	root := repoRoot(t)
	fixturesDir := filepath.Join(root, "tests", "transpiler3", "beam", "fixtures", "phase07")
	...
	if ran == 0 {
		t.Fatal("no fixtures found in phase07/")
	}
}
