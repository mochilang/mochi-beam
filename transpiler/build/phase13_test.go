...
// TestPhase13Builtins runs the Phase 13 builtin (string/math/list) fixture suite.
func TestPhase13Builtins(t *testing.T) {
	root := repoRoot(t)
	fixturesDir := filepath.Join(root, "tests", "transpiler3", "beam", "fixtures", "phase13")
	...
	if ran == 0 {
		t.Fatal("no fixtures found in phase13/")
	}
}
