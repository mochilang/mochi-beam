...
// TestPhase9Agents runs the Phase 9 fixture suite.
// It walks tests/transpiler3/beam/fixtures/phase09/ and asserts that
// each fixture's escript stdout matches expect.txt byte-for-byte.
func TestPhase9Agents(t *testing.T) {
	root := repoRoot(t)
	fixturesDir := filepath.Join(root, "tests", "transpiler3", "beam", "fixtures", "phase09")
	...
	if ran == 0 {
		t.Fatal("no fixtures found in phase09/")
	}
}
