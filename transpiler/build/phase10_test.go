...
// TestPhase10Streams runs the Phase 10 fixture suite.
// It walks tests/transpiler3/beam/fixtures/phase10/ and asserts that
// each fixture's escript stdout matches expect.txt byte-for-byte.
func TestPhase10Streams(t *testing.T) {
	root := repoRoot(t)
	fixturesDir := filepath.Join(root, "tests", "transpiler3", "beam", "fixtures", "phase10")
	...
	if ran == 0 {
		t.Fatal("no fixtures found in phase10/")
	}
}
