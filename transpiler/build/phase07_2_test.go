...
// TestPhase7_2GroupBy runs the Phase 7.2 fixture suite.
// It walks tests/transpiler3/beam/fixtures/phase07_2/ and asserts that
// each fixture's escript stdout matches expect.txt byte-for-byte.
func TestPhase7_2GroupBy(t *testing.T) {
	root := repoRoot(t)
	fixturesDir := filepath.Join(root, "tests", "transpiler3", "beam", "fixtures", "phase07_2")
	...
	if ran == 0 {
		t.Fatal("no fixtures found in phase07_2/")
	}
}
