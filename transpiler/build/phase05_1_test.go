...
// TestPhase5_1Guards runs the Phase 5.1 fixture suite.
// It walks tests/transpiler3/beam/fixtures/phase05_1/ and asserts that
// each fixture's escript stdout matches expect.txt byte-for-byte.
func TestPhase5_1Guards(t *testing.T) {
	root := repoRoot(t)
	fixturesDir := filepath.Join(root, "tests", "transpiler3", "beam", "fixtures", "phase05_1")
	...
	if ran == 0 {
		t.Fatal("no fixtures found in phase05_1/")
	}
}
