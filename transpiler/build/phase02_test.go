...
// TestPhase2Primitives runs the Phase 2 fixture suite.
// It walks tests/transpiler3/beam/fixtures/phase02/ and asserts that
// each fixture's escript stdout matches expect.txt byte-for-byte.
func TestPhase2Primitives(t *testing.T) {
	root := repoRoot(t)
	fixturesDir := filepath.Join(root, "tests", "transpiler3", "beam", "fixtures", "phase02")
	...
	if ran == 0 {
		t.Fatal("no fixtures found in phase02/")
	}
}
