...
// TestPhase5SumTypes runs the Phase 5 fixture suite.
// It walks tests/transpiler3/beam/fixtures/phase05/ and asserts that
// each fixture's escript stdout matches expect.txt byte-for-byte.
func TestPhase5SumTypes(t *testing.T) {
	root := repoRoot(t)
	fixturesDir := filepath.Join(root, "tests", "transpiler3", "beam", "fixtures", "phase05")
	...
	if ran == 0 {
		t.Fatal("no fixtures found in phase05/")
	}
}
