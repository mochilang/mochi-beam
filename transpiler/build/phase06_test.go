...
// TestPhase6Closures runs the Phase 6 fixture suite.
// It walks tests/transpiler3/beam/fixtures/phase06/ and asserts that
// each fixture's escript stdout matches expect.txt byte-for-byte.
func TestPhase6Closures(t *testing.T) {
	root := repoRoot(t)
	fixturesDir := filepath.Join(root, "tests", "transpiler3", "beam", "fixtures", "phase06")
	...
	if ran == 0 {
		t.Fatal("no fixtures found in phase06/")
	}
}
